...
// Выключать использование блоков в редакторе
...
// Добавление файла обнуления для блоков
add_action('admin_enqueue_scripts', function () {
	wp_enqueue_style(
		'fls-blocks-admin-reset-styles', // уникальный идентификатор
		get_template_directory_uri() . '/components/blocks/admin/admin-reset.css', // путь к файлу
		[],
		'0'
	);
	wp_enqueue_style(
		'fls-blocks-admin-common-styles', // уникальный идентификатор
		get_template_directory_uri() . '/components/blocks/admin/dist/css/admin-common.css', // путь к файлу
		[],
		'0'
	);
});
...
// Реакция на изменения в админке
add_action('save_post', function ($post_id) {
	// не реагировать на автосейв
	//  if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
	// не реагировать на ревизии
	//  if (wp_is_post_revision($post_id)) return;
	// (опционально) только для постов/страниц
